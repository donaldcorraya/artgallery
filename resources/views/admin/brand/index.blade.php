...
@section('title', 'Brand list| Art Gallery')
@section('content')
    <style>
        .img_image {
            width: 120px;
    ...
    <main id="main" class="main">
        <div class="d-flex justify-content-between">
            ...
            <div class="text-end pt-2">
                <a href="{{ route('brand.create') }}" class="btn btn-sm btn-dark"><i class="fas fa-plus-circle"></i>
                    Add
                    Data</a>
            </div>
        </div>
                ...
                <table id="myTable" class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($indexData as $item)
                            <tr>
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->brand_tagline }}</td>
                                <td>
                                    <img src="/images/{{ $item->brand_img }}" alt="Image not found"
                                        style="height: 40px; width: 40px;" class="rounded">
                                </td>
                                <td class="d-flex icon">
                                    <a href="{{ route('brand.show', $item->id) }}" type="button" class="btn view"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a href="{{ route('brand.edit', $item->id) }}" type="button" class="btn edit"><i
                                            class="fa-solid fa-pen text-warning"></i></a>
                                    <a href="{{ route('brand.destroy', $item->id) }}" type="button"
                                        class="btn delete" onclick="return confirm('Are you sure dalete')"><i
