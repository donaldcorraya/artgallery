...
@section('title', 'Main Banner list | Art Gallery')
@section('content')
    <style>
        .img_image {
            width: 120px;
                ...
                <table id="myTable" class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($indexData as $item)
                            <tr>
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->main_banner_title }}</td>
                                <td>{{ $item->main_banner_tagline }}</td>
                                <td>
                                    <img src="/images/{{ $item->main_banner_img }}" alt="Image not found"
                                        style="height: 40px; width: 40px;" class="rounded">
                                </td>
                                <td class="d-flex icon">
                                    <a href="{{ route('main_banner.show', $item->id) }}" type="button" class="btn view"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a href="{{ route('main_banner.edit', $item->id) }}" type="button" class="btn edit"><i
                                            class="fa-solid fa-pen text-warning"></i></a>
                                    <a href="{{ route('main_banner.destroy', $item->id) }}" type="button"
