...
@section('title', 'Top Banner list | Art Gallery')
@section('content')
    <style>
        .img_image {
            width: 120px;
    ...
    <main id="main" class="main">
        <div class="d-flex justify-content-between">
            ...
            <div class="text-end pt-2">
                <a href="{{ route('top_banner.create') }}" class="btn btn-sm btn-dark"><i class="fas fa-plus-circle"></i>
                    Add
                    Data</a>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach ($indexData as $item)
                            <tr>
                                ...
                                <td>{{ $item->top_banner_tagline }}</td>
                                <td>
                                    <img src="/images/{{ $item->top_banner_img }}" alt="Image not found"
                                        style="height: 40px; width: 40px;" class="rounded">
                                </td>
                                <td class="d-flex icon">
                                    <a href="{{ route('top_banner.show', $item->id) }}" type="button" class="btn view"><i class="fa-solid fa-eye"></i></a>
                                    <a href="{{ route('top_banner.edit', $item->id) }}" type="button" class="btn edit"><i
                                            class="fa-solid fa-pen text-warning"></i></a>
                                    <a href="{{ route('top_banner.destroy', $item->id) }}" type="button" class="btn delete"
